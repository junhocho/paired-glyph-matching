...
class PUILoss(nn.Module):
    ...
    def forward(self, x, y):
        ...
        x : batchsize x head_num
        y : batchsize x head_num
        pui : head_num x head_num

        """

        # norm_p = cfg.pui_norm_p
        norm_p = 2

        if norm_p > 0:
            pui_x = F.normalize(x.t(), p=norm_p, dim=1)  # norm in batchsize
            pui_y = F.normalize(y, p=norm_p, dim=0)      # norm in batchsize 
        else:
            pui_x = x.t()
            pui_y = y
        pui = torch.mm(pui_x, pui_y)   # head_num x head_num 
        # torch.arange(pui.size(0)) : [0, 1, ..., 69 ]
