...
class DefaultModel(nn.Module):
    ...
    def load_state_dict(self, state_dict):
        ...
        # state_dict = {key.replace('module.', ''):val for key,val in state_dict.iteritems()} ## XXX Python2
        state_dict = {key.replace('module.', ''):val for key,val in state_dict.items()} ## use item
        # get valid params
        target = self.state_dict()
        cnt = 0
        for key, val in state_dict.items():
            # XXX : this supports SimCLR pretrained from SCAN
            if key.startswith('backbone.'):
                key = key[9:]

            if key not in target:
                print('Ignoring pretrained weights [%s]: not found' % key)
            elif val.shape != target[key].shape:
                print('Ignoring pretrained weights [%s]: invalid shape '
                    '[%s] v.s. [%s]' % (key, val.shape, target[key].shape))
            else:
                target[key] = val
                cnt += 1
                print('Successfully loaded pretrained weights [%s]: ' % key)
        print('Totally loaded [%d] parameters' % cnt)
